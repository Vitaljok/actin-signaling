...
def build_rules(variant):
    ...
    if variant != 'EQ':
        G.add_weighted_edges_from(get_edges(Rac1, [Arp2_3, Dia1], [ARHGAP22]))
        rules += "1: Rac1* = not ARHGAP22 and (Arp2_3 or Dia1) \n" 
        
    G.add_weighted_edges_from(get_edges(ROS, [Rac1], []))
    rules += "1: ROS* = Rac1 \n" 
    ...
    G.add_weighted_edges_from(get_edges(Dia2, [Cdc42, RhoA], []))
    ...
    rules += "1: Profilin* = " + voting_rule(["WASP", "Dia1", "WAVE"], ["(Thymosin and PIP2)"]) + " \n"

    G.add_weighted_edges_from(get_edges(CP, [Arp2_3], [Ena_Vasp, PIP2]))
    # rules += "1: CP* = (Arp2_3 and not Ena_Vasp) or (not PIP2 and not Ena_Vasp) \n"
    rules += "1: CP* = " + voting_rule(["Arp2_3"], ["PIP2", "Ena_Vasp"]) + " \n"
    ...
    G.add_weighted_edges_from(get_edges(ARHGAP22, [ROCK], []))
    rules += "1: ARHGAP22* = ROCK \n"
    ...
    return {"rules":rules, "graph":G, "group": group}
